...
<style>
    ...
    .inventory-button { position: relative; display: inline-flex; align-items: center; gap: 0.5rem; padding: 0.65rem 1rem; border: 2px solid var(--neutral-200); border-radius: 0.75rem; background: white; color: var(--neutral-700); font-weight: 600; cursor: pointer; transition: all 0.2s; }
    .inventory-button:hover { border-color: var(--primary); color: var(--primary); box-shadow: 0 6px 12px rgba(30, 58, 95, 0.12); }
    .inventory-button svg { width: 18px; height: 18px; }
    .inventory-badge { position: absolute; top: -8px; right: -8px; min-width: 22px; height: 22px; padding: 0 6px; border-radius: 999px; background: var(--accent); color: white; font-size: 0.75rem; font-weight: 700; display: none; align-items: center; justify-content: center; }
    ...
    .product-actions { display: grid; grid-template-columns: repeat(3, 1fr); gap: 0.5rem; }
    ...
    .cart-item { display: grid; grid-template-columns: auto 1fr auto; gap: 1rem; align-items: center; padding: 0.75rem; border: 1px solid var(--neutral-200); border-radius: 0.75rem; background: var(--neutral-50); }
    .cart-item-info { display: flex; flex-direction: column; gap: 0.25rem; }
    .cart-item-name { font-weight: 700; color: var(--neutral-800); }
    .cart-item-meta { font-size: 0.85rem; color: var(--neutral-500); }
    .cart-item-actions { display: flex; align-items: center; gap: 0.75rem; }
    .cart-qty-input { width: 70px; padding: 0.35rem 0.5rem; border: 2px solid var(--neutral-200); border-radius: 0.5rem; font-size: 0.9rem; text-align: center; }
    .cart-item-total { font-weight: 700; color: var(--primary); min-width: 110px; text-align: right; }
    .cart-remove { border: none; background: rgba(220, 38, 38, 0.1); color: var(--error); padding: 0.35rem 0.6rem; border-radius: 0.5rem; font-size: 0.8rem; font-weight: 600; cursor: pointer; }
    .cart-remove:hover { background: var(--error); color: white; }
    .cart-summary { margin-top: 1.25rem; padding: 1rem; border-radius: 0.75rem; background: var(--neutral-50); border: 1px solid var(--neutral-200); }
    .cart-summary-row { display: flex; justify-content: space-between; font-weight: 600; color: var(--neutral-700); margin-bottom: 0.5rem; }
    .cart-summary-row:last-child { margin-bottom: 0; }
    .cart-empty { text-align: center; padding: 2rem; color: var(--neutral-500); }
    .cart-notes { margin-top: 1.5rem; }
    ...
    <main class="main-content">
        ...
        <div class="content-header">
            ...
            <div class="header-actions">
                ...
                <button type="button" class="inventory-button" id="inventoryButton" aria-label="Inventory">
                    <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round">
                        <path stroke-width="2" d="M7 4h6a2 2 0 012 2v1h-10V6a2 2 0 012-2Z" />
                        <path stroke-width="2" d="M5 7h14a2 2 0 012 2v11a2 2 0 01-2 2H5a2 2 0 01-2-2V9a2 2 0 012-2Z" />
                        <path stroke-width="2" d="M7 13h2" />
                        <path stroke-width="2" d="M11 13h5" />
                        <path stroke-width="2" d="M7 17h2" />
                        <path stroke-width="2" d="M11 17h5" />
                        <path stroke-width="2" d="M6.5 19.5L5 20l.5-1.5 7.75-7.75 1.5 1.5-7.75 7.75Z" />
                        <path stroke-width="2" d="M13.75 11.75l1.5-1.5 1.75 1.75-1.5 1.5-1.75-1.75Z" />
                        <path stroke-width="2" d="M10.5 8.5a1.5 1.5 0 1 1 3 0a1.5 1.5 0 0 1-3 0Z" />
                    </svg>
                    Inventory
                    <span class="inventory-badge" id="inventoryCount">0</span>
                </button>
                    ...
                    <div class="product-card" data-stock-status="{{ $product->quantity <= 0 ? 'out' : ($product->quantity < 10 ? 'low' : 'in') }}">
                        ...
                        <div class="product-info">
                            ...
                            <div class="product-actions">
                                <button class="action-btn sell" title="Add To Cart" data-action="add-to-cart" data-product-id="{{ $product->id }}" data-product-name="{{ $product->name }}" data-product-price="{{ $product->price }}" data-product-stock="{{ $product->quantity }}" {{ $product->quantity <= 0 ? 'disabled' : '' }}>
                                    <svg fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <form id="addForm" method="POST" action="{{ route('products.store') }}" enctype="multipart/form-data">
            ...
            <div class="modal-body">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        <label for="add_sku">SKU (Stock Keeping Unit) <span style="color: red">*</span></label>
        ...
        <form id="editForm" method="POST" enctype="multipart/form-data">
            ...
            <div class="modal-body">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        <label for="edit_sku">SKU (Stock Keeping Unit) <span style="color: red">*</span></label>
    ...
    function injectProductCard(product) {
        ...
        card.innerHTML = `
            ...
            <div class="product-info">
                ...
                <div class="product-actions">
                    <button class="action-btn sell" title="Add To Cart" data-action="add-to-cart"
                        data-product-id="${product.id}" data-product-name="${escHtml(product.name)}"
                        data-product-price="${product.price}" data-product-stock="${qty}"
                        ${qty <= 0 ? 'disabled' : ''}>
